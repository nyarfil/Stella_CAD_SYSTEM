...
- **AgentCAD**（`agentcad-for-windows/`）: 部品は build123d、判定はカーネル。本家と同じ工具を MCP 名 **`Stella_Agentcad`** で出します。既定の一覧は案内役 `stella_guide` と実行 `stella_run` だけ（`STELLA_MCP_SURFACE=core`）。全部出すのは `all`。
...
MCP の `Stella_Agentcad` が繋がっているときは、**先に `stella_guide` を呼ぶ**のが正です。やりたいことを渡すと、今回使ってよい工具だけ（名前と引数の形）が返ります。**それ以外は呼ばない。** 実行は全部 `stella_run`。既定では 109 個は一覧に出ません（`STELLA_MCP_SURFACE=core`）。全部欲しいときだけ `STELLA_MCP_SURFACE=all`。

最初の部品を書く流れ（案内役がこれを返したとき）:
