"""30 x 30 T-slot aluminium extrusion, the 8 mm-slot (metric "30 series")
profile.

An **interface model**: the 30 x 30 envelope, the four 8 mm slot openings, the
T-channels behind them and the 6.8 mm centre bore are the dimensions a design
...
meet: the profile falls into five loose pieces that still measure 30 x 30 and
still have a 6.8 mm bore. That is exactly what this part used to build (five
solids, 382.93 mm^2 of section), so ``one_connected_solid`` and
...
SPECS measure the built solid: it is one connected bar, the section is 30 x 30
...
6.8 mm (the tapping size for an M8 end fastener, which is the whole reason it
is there).
"""
...
#: convention, not a standard** — these are the commodity metric "30 series /
#: slot 8" dimensions.
SIZE = 30.0
SLOT_OPEN = 8.0
LIP_T = 2.0
...
#: T-slot profiles, so the reference is mass: commodity 30-series slot-8 bar is
#: published at roughly **0.95-1.25 kg/m**, and at 6063 aluminium's
#: **2.70 g/cm^3** that is 352-463 mm^2. Rounded outward to 350-465. This model
#: measures **404.99 mm^2 (1.094 kg/m)** — mid-band, and it would be a little
#: lighter still with the corner chamfers and web fillets it does not model.
SECTION_MIN_MM2 = 350.0
SECTION_MAX_MM2 = 465.0
...
def _channel_void():
    ...
    * ``(bx, +/- by)`` — where that web face meets the boss circle.
    ...
    `slot_inner / 2` (8.25) is larger than `size / 2 - slot_depth` (6.5).
    """
    half = SIZE / 2.0
...
def _quarter_turns(points, turns):
    ...
    nothing** for one of the four faces on the 20-series profile (400 ->
    351.51 -> 351.51 mm^2, no error raised): OCCT succeeding is not evidence,
    so the rotation that is exact is the one that ships.
    """
...
def _section_is_30_square(part, metrics) -> bool:
    bbox = metrics["bbox"]
...
def _centre_bore_is_6_8(part, metrics) -> bool:
...
SPECS = [
    check_valid(requirement="EXT3030-01"),
    check_that(_section_is_30_square, name="section_30x30",
               requirement="EXT3030-02"),
    check_that(_length_is_the_length_asked_for, name="length_and_origin",
               requirement="EXT3030-03"),
    check_that(_centre_bore_is_6_8, name="centre_bore",
               requirement="EXT3030-04"),
    check_that(_is_one_connected_bar, name="one_connected_solid",
               requirement="EXT3030-05"),
    check_that(_section_area_is_plausible, name="section_area",
               requirement="EXT3030-06"),
]
