...
def cmd_health(args) -> int:
    status, payload = http_json("GET", f"{base_url(args.port)}/api/health", timeout=5.0)
    if status is None:
...
def _health_payload(port: int | None):
    status, payload = http_json("GET", f"{base_url(port)}/api/health", timeout=5.0)
    if status is None:
        return None
    return payload
...
def cmd_stop(args) -> int:
    ...
    if pid is None:
        return emit({
            "stopped": False,
            "reason": "no pid file; refusing to kill an unknown process",
            "pid_file": str(pid_file()),
        })

    if not _process_alive(pid):
        pid_file().unlink(missing_ok=True)
        return emit({"stopped": False, "reason": "pid not running", "pid": pid})

    # Only the process we recorded. Never scan the machine for "python".
    subprocess.run(
    ...
    pid_file().unlink(missing_ok=True)
    return emit({"stopped": True, "pid": pid})
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_serve.add_argument("--wait", type=int, default=180)
