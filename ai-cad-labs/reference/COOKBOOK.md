...
> **Purpose:** Reusable code patterns extracted from golden examples in the upstream
> CadQuery ecosystem repos (`Source:` lines below are upstream provenance, not local paths).
> Copy and adapt these templates instead of writing CadQuery from scratch.
>
> For Claude agents: When generating CadQuery code, check this cookbook first.
...
**Why**: When both a shaft and its housing have their axes at Z-origin, assembly
is just `cq.Location(cq.Vector(0, 0, z_offset))`. No guessing where features are
relative to part corners.
...
A sub-assembly = parts that are ALWAYS assembled together before joining the
larger assembly (e.g., connecting rod + big end cap + bolts).
...
- Sub-assembly `assembly.py` uses the same `load_part()` pattern but with paths
  relative to `__project_path__ / "assembly" / sub_name / part_name / "part.py"`
