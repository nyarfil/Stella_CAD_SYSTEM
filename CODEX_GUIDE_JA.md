...
更新: cadMCP Design Studio 0.3.3を正式版として運用。CodexのプロジェクトMCPと製品Skillを有効化し、現アプリから4診断の実呼出し、隔離stdio50工具、Factory OSの個人能力カタログ登録を確認。最新手順は `docs/MCP_PREVIEW_JA.md`（旧ファイル名を互換維持）。Cursorは安全境界により無効のまま、グローバル・信頼設定は変更していません。

このリポジトリには正式版0.3.3のCodex用MCP設定と設計Skillがあります。CursorのcadMCP設定は安全境界により無効化しています。元設定のバックアップと無関係なユーザー設定は保持しています。
...
本体のソフトウェア開発・保守では製品CADワークフローを実行せず、接続診断と製品利用を分離します。既存セッションの工具一覧だけで接続済みと推定しません。
...
新規Codexサーバ設定は安全のため無効を初期値とします。この正式版プロジェクト設定では所有者の明示判断により有効化しています。パスはこのPC専用です。

## 検証記録

### 現在の到達点（2026-09-20）

- 正式版のCodexプロジェクトMCPと製品Skillは有効。隔離した通信試験は50工具で成功し、現アプリの読取診断も別証拠として保存している。GUI接続・CAD品質・物理性能は別判定。
